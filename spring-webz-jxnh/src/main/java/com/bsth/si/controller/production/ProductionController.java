package com.bsth.si.controller.production;
...
public class ProductionController extends BaseController<Production> {
	...
	public ModelAndView index(@RequestParam Map<String, Object> map) {
		...
		andView.setViewName("/pages/production/list.jsp");
	...
	public ModelAndView edit(@RequestParam Map<String, Object> map) {
		...
		ModelAndView andView = new ModelAndView();
		andView.addAllObjects(map);
		andView.setViewName("/pages/production/edit.jsp");
		logger.debug(andView);
		return andView;
	}
	...
	Map<String, Object> check(@RequestParam Map<String, Object> map) {
		...
		try {
			...
			List<Production> checkList = service.query(map);
			int count = 0;
			if(checkId.equals("")){
				count= checkList.size();
			}else{
				for (Production temp : checkList) {
