...
public class ContextLoader {
	 ...
	 */
	/**
	 * 根据web应用的servletContext 初始化WebApplicationContext
	 *
	 * @param servletContext
	 * @return
	 */
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		//查询servletContext是否已经有了WebApplicationContext root,以此判断web.xml是否重复配置了ContextLoaderListener
		...
		try {
			...
			//如果WebApplicationContext对象不存在就根据servletContext生成一个
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		//根据servlet中配置的contextClass获取context classd对象，若未配置则默认为XmlWebApplicationContext的class对象
		...
		//创建WebApplicationContext对象并强制转换为ConfigurableWebApplicationContext类型
	 ...
	 */
	/**
	 * 生成(WebApplicationContext)class对象，若web.xml中配置了则根据配置生成，若没有配置则默认生成XmlWebApplicationContext的class对象
	 * 缓存中检索，无法检索到就直接生成
	 * @param servletContext
	 * @return
	 */
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		//若WebApplicationContext的id与classname+@+hashcode一致就更新WebApplicationContext的id
		//id首先去web.xml中配置的contextId，若没有系统生成
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			// The application context id is still set to its original default value
			// -> assign a more useful id based on available information
			//查询webxml中是否配置contextId
		...
		//将web.xml中的配置（contextConfigLocation）存放到ApplicationContext中（放在AbstractRefreshableConfigApplicationContext的configLocations中）
		//configLocations是一个string数组，web.xml配置的contextConfigLocation以，或其他分割的多文件会被解析成数组
		...
		//获取WebApplicationContext环境对象，然后将ServletContext初始化到环境对象中
		ConfigurableEnvironment env = wac.getEnvironment();
		if (env instanceof ConfigurableWebEnvironment) {
			((ConfigurableWebEnvironment) env).initPropertySources(sc, null);
		}
		//在刷新之前执行web.xml中配置的 执行web.xml配置的ApplicationContextInitializer实现类的initialize方法
		customizeContext(sc, wac);
		//WebApplicationContext容器刷新
		wac.refresh();
	}
	 ...
	 */
	/**
	 * 执行web.xml配置的ApplicationContextInitializer实现类的initialize方法
	 * 若web.xml配置了就执行，若没配置则不执行。
	 * @param sc
	 * @param wac
	 */
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		//根据web.xml配置生成ApplicationContextInitializer的class对象list集合
		...
		for (Class<ApplicationContextInitializer<ConfigurableApplicationContext>> initializerClass : initializerClasses) {
			...
			//根据ApplicationContextInitializer的类对象创建实例对象并存放到list集合中
	...
	/**
	 * 根据web.xml配置生成ApplicationContextInitializer的class对象list集合
	 * web.xml的配置名为 globalInitializerClasses 或 contextInitializerClasses，配置的必须是ApplicationContextInitializer类型的类名
	 * @param servletContext
	 * @return
	 */
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		//从web.xml中获取globalInitializerClasses配置值(ApplicationContextInitializer类名)
		String globalClassNames = servletContext.getInitParameter(GLOBAL_INITIALIZER_CLASSES_PARAM);
		if (globalClassNames != null) {
			for (String className : StringUtils.tokenizeToStringArray(globalClassNames, INIT_PARAM_DELIMITERS)) {
				//将globalInitializerClasses转换为ApplicationContextInitializer类对象
				classes.add(loadInitializerClass(className));
			}
		}
		//获取web.xml配置的contextInitializerClasses(ApplicationContextInitializer类名)
	...
	/**
	 * 根据类名加载ApplicationContextInitializer类对象
	 * @param className
	 * @return
	 */
	@SuppressWarnings("unchecked")
